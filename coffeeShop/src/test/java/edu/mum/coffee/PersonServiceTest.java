...
public class PersonServiceTest {
	...
	public void testSavePerson() {
		...
		Person saved = personService.getPerson(stored.getId());
	...
	public void testFindByEmail() {
		List<Person> persons = personService.getPersonByEmail("SOMETHING ELSE");
		if (!persons.isEmpty()) {
			fail("Not expected to match Person");
		}
		for (Person saved : personService.getPersonByEmail(testPerson[1].getEmail())) {
	...
	public void testFindById() {
		...
		Person saved = personService.getPerson(something_else);

		if (comparePerson(testPerson[1], saved)) {
			fail("Not expected to match Person");
		}

		saved = personService.getPerson(testPerson[2].getId());
	...
	private void deleteTestObjects() {
		for (int i = 0; i < TEST_NumberOfPersons; i++) {
			try {
				Person person = personService.getPerson(testPerson[i].getId());
